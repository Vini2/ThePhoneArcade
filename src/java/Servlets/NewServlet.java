...
public class NewServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String ss = "";
            String st = request.getParameter("category");
            
            Session s = PoolManager.getSessionFactory().openSession();
            
            Criteria c = s.createCriteria(Category.class);
            c.add(Restrictions.eq("idcategory", Integer.parseInt(st)));
            Category cat = (Category) c.uniqueResult();
            
            Criteria c1 = s.createCriteria(Product.class);
            c1.add(Restrictions.eq("category", cat));
            List<Product> lp = c1.list();
            
            for (Product p : lp) {
                ss+="<option value="+p.getIdproduct()+">"+p.getName()+"</option>";
            }
